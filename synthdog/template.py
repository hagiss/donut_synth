...
class SynthDoG(templates.Template):
    ...
    def generate(self):
        ...
        # print(document_group.layers[0].quad)
        quads = [layer.quad for layer in document_group.layers] # list of [[[x1, y1], [x2, y2], [x3, y3], [x4, y4]], .....]
        # get bbox from quads [[min_x, min_y, max_x, max_y], .....]
        bboxes = [[min(quad[:, 0]), min(quad[:, 1]), max(quad[:, 0]), max(quad[:, 1])] for quad in quads]
    ...
    def save(self, root, data, idx):
        ...
        metadata = self.format_metadata(image_filename=image_filename, keys=["text_sequence", "bbox", "texts"], values=[label, str(bbox), str(texts)])
